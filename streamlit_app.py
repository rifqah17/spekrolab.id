...
# -------------------- Halaman Upload Gambar Spektrum --------------------
    ...
    if uploaded_file is not None:
        ...
        if angka_unik:
            ...
            for p in angka_unik:
                match = next((f"➡ {p} cm⁻¹: {gugus} ({desc})" for gugus, start, end, desc in ir_data if start <= p <= end), None)
                st.write(match or f"➡ {p} cm⁻¹: Belum terdaftar.")
        else:
...
# -------------------- Halaman Input Manual --------------------
    ...
    if panjang:
        try:
            ...
            match = next((f"🟢 {gugus} ({desc})" for gugus, start, end, desc in ir_data if start <= p <= end), None)
            st.write(match or "⚠ Data tidak ditemukan di database.")
        except ValueError:
